...
$$("#btn-search").on('click',function(e)
{
    ...
    var pageName = "searchresult?shade_name="+shadeName;
...
function initMap() {
    ...
    var location = { lat:40.23, lng:-120.4 };
...
$$('.btn-schedule').click(function(e){
    myApp.alert("We will contact you around the time","Schedule",function(){
    });
});
...
function find_products(shade_name)
{
    myApp.showIndicator();
    $$.ajax({
        url: base_url+'?mode=searchproducts&&shade_name='+shade_name,
        type: 'GET',
        dataType: 'json',
        success: function(data) {
            $$(".search-list ul").html('');
            if (data.length > 0)
            {
                ...
                for (var i = 0; i < data.length; i++) {
                    var product_info = JSON.parse(data[i].product);
                    var image_url=filter_slash(product_info.image);
                    if(image_url.substr(0,4)!='http')
                        image_url=findation_url+image_url;
                    var htmlObj = '<li>' +
                        '<a href="#product?url=' + filter_slash(product_info.url) + '" class="item-link">' +
                            '<div class="item-content">'+
                        '<div class="item-media"><img src="' + image_url + '"/></div>' +
                        '<div class="item-inner"><div class="item-title-row"><div class="item-title">' + filter_slash(product_info.title) + '</div></div>' +
                        '<div class="item-text">' + filter_slash(product_info.content) + '</div>' +
                        '</div></div>' +
                        '</a>' +
                        '</li>';
